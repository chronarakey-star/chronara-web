...
export default function SellModule({ companyId, storeId, themeColor, user, setActiveModule, refundData, clearRefundData }: SellModuleProps) {
  ...
  const saveTransactionToDatabase = async (finalPaymentQueue: any[], changeDue: number = 0) => {
    try {
          ...
          for (const orig of originalSaleItems) {
             ...
             allSkus.forEach(sku => {
                 ...
                 finalIngredients.push({
                     ...
                     is_damaged: refundIngredientShrinkageIds.has(`${orig.line_id}:${sku}`) // <--- THE FIX: Read from the immutable Set, not the UI Cart
                 });
             });
      ...
      // 3. Prepare Sale Items (WITH ID SPACING AND GC SNAPSHOT FIX)
      ...
      // 4. Prepare Sale Payments (WITH ID SPACING FIX)
      ...
      // --- EXECUTE CORE INSERTS WITH EXPLICIT ERROR TRACKING ---
      ...
      // 8. Inventory Ledger & Direct Products Update (Mirrors Python Logic)
      ...
      for (const item of itemsToSave) {
        ...
        if (item.ingredients && item.ingredients.length > 0) {
          for (const ing of item.ingredients) {
            ...
            // THE FIX: Intercept damaged child items and block restock
            ...
            if (ingQtyChange !== 0) {
              ...
              productUpdates.push({ sku: ingSku, qty_change: ingQtyChange });
            }
          }
        } else {
          // THE FIX: Intercept damaged parent items and block restock
          ...
          if (parentQtyChange !== 0) {
            ...
            productUpdates.push({ id: item.id, qty_change: parentQtyChange });
          }
        }
      }
      ...
      // Execute Direct Product Updates 
      for (const update of productUpdates) {
        let query = supabase.from('products').select('id, quantity').eq('company_id', companyId);
        
        if (update.id) {
            query = query.eq('id', update.id);
        } else if (update.sku) {
            query = query.eq('sku', update.sku);
            if (finalStoreId) query = query.eq('store_id', finalStoreId);
        ...
        if (prodData) {
            const newQty = parseFloat(prodData.quantity || 0) + update.qty_change;
            await supabase.from('products').update({ quantity: newQty }).eq('id', prodData.id);
        }
      }
  ...
  const processPayment = async (method: string) => {
    ...
    if (method === "Gift Card") {
      // THE FIX: Uses the custom promise-based prompt
      cardNum = await customPrompt("Gift Card Payment", "Swipe or Enter Gift Card Number:");
      
      if (!cardNum || !cardNum.trim()) return; // User clicked cancel or left it blank
    ...
    // CAP GIFT CARD CHARGE TO AVAILABLE BALANCE
