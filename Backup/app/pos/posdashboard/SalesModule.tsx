    ...
    try {
      ...
      for (const p of salePayments) {
          ...
          if (methodStr === "gift card" && p.payment_ref) {
              ...
              // 2. Safely verify if this was the last active transaction (2-step check bypasses strict FK requirements)
              ...
              if (otherPayments && otherPayments.length > 0) {
                  ...
                  if (count === 0) {
                      // No active sales tie back to this payment
                      await supabase.from('gift_cards').update({ is_used: 0, updated_at: timestampStr }).eq('company_id', companyId).eq('card_number', cNum);
                  }
              } else {
                  // No other payments found at all
                  await supabase.from('gift_cards').update({ is_used: 0, updated_at: timestampStr }).eq('company_id', companyId).eq('card_number', cNum);
              }
          }
      }

      // 1. THE LEDGER MATH: Restock valid inventory & Direct Product Updates
      const inventoryRecords: any[] = [];
      const productUpdates: any[] = [];

      saleItems.forEach((item) => {
        // Skip damaged items or system SKUs by safely checking the SKU and Name
        ...
        if (qtyToRestore !== 0) {
          // Try to parse ingredients from the snapshot
          let parsedIngredients = [];
          if (item.ingredients_snapshot) {
            try {
              const snap = JSON.parse(item.ingredients_snapshot);
          ...
          if (parsedIngredients.length > 0) {
            // Restore child ingredients
            parsedIngredients.forEach((ing: any) => {
              if (ing.is_damaged) return; // Don't restock damaged sub-ingredients
              ...
              if (ingQtyToRestore !== 0) {
                ...
                productUpdates.push({ sku: ingSku, qty_change: ingQtyToRestore });
              }
            });
          } else {
            // Standard item, restore the parent
            inventoryRecords.push({
              id: `LEDG_${crypto.randomUUID().replace(/-/g, "").substring(0, 16)}`,
              company_id: companyId,
              store_id: selectedSale.store_id || null,
            ...
            productUpdates.push({ id: item.product_id, sku: item.sku, qty_change: qtyToRestore });
          }
        }
      });
      ...
      for (const update of productUpdates) {
        ...
        if (update.id) {
            ...
            if (selectedSale.store_id) query = query.eq('store_id', selectedSale.store_id);
